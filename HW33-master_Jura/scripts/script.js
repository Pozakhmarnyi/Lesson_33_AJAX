...
let searchData = {
  ...
  totalPages: function () {
    return (
      Math.trunc(this.totalResults / 10) + (this.totalResults % 10 ? 1 : 0)
    );
  },
};
...
function printSearch(search, type, page) {
  onOffLoading(true);
  fetch(
    `http://www.omdbapi.com/?apikey=${apiKey}&s=${strToAddr(search)}${
      type ? "&type=" + type : ""
    }&page=${page}`
  )
    ...
    .then(function (response) {
      ...
      $(".movie").click(function (event) {
        ...
        fetch(
          `http://www.omdbapi.com/?apikey=${apiKey}&i=${$(this).attr("id")}`
        )
          .then(function (response) {
            return response.json();
          })
...
$(window).scroll(function (event) {
  if (
    $(window).scrollTop() + $(window).height() * 1.1 >= $(document).height() &&
    load
  ) {
    console.log("yes");
